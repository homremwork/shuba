...
В [`release/release.properties`](release/release.properties:1) задан кандидат публикации: версия `1.0.2`, Android version code `3`, Android 14/API 34 и только `arm64-v8a`. Нативная политика собирает приложение и статические зависимости для `cortex-a73`; требуемый нижний набор возможностей — Armv8-A с NEON, AES, SHA2 и CRC32. Это **не** политика Armv8.2-A: такой минимальный ISA противоречил бы заявленной совместимости с классом Cortex-A73/A53 и совместимыми Snapdragon 680/685 Kryo 265. Debug использует `-O0`, Release — безопасный `-O3`; `-Ofast`, fast-math и LTO не используются. Локальные базовая и smoke-проверки на Android 14 приняты. Существующая внешняя проверка остаётся доказательством для текущего финального исходного кода до её сравнения с точными байтами размещённого APK; ни одно предыдущее доказательство не разрешает выпуск изменённого текущего исходного кода. До публикации необходимы новый подписанный кандидат и Android-проверка затронутой функциональности. Перед сборкой или распространением APK прочитайте [`docs/release.md`](docs/release.md).
...
The tracked application authority identifies the publication candidate as version `1.0.2`, Android version code `3`, Android 14/API 34, and `arm64-v8a` only in [`release/release.properties`](release/release.properties:1). The native policy builds both the app and static dependencies for `cortex-a73`, with an Armv8-A + NEON + AES + SHA2 + CRC32 feature floor. It is explicitly **not** an Armv8.2-A minimum: that ISA would contradict the advertised Cortex-A73/A53 and compatible Snapdragon 680/685 Kryo 265 device class. Debug uses `-O0`; Release uses safe `-O3`; `-Ofast`, fast-math, and LTO are not used. Local Android 14 base and smoke testing is accepted. Existing external testing remains evidence for the current final runtime source until it is compared with the exact hosted APK bytes; no earlier evidence authorizes the changed current source as a release. A new signed candidate and affected Android acceptance remain required before publication. Read [`docs/release.md`](docs/release.md) before building or distributing an APK.
