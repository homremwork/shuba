...
function shuba_ci_release_tests_main
    ...
    set --local shuba_expected_tests \
        ...
        test-publication-assets.fish test-signing-preparer.fish test-upgrade-probe.fish
    ...
    if contains -- $shuba_mode hermetic rehearsal
        for shuba_test in \
            test-agp-metadata.fish test-apk-validation.fish test-atomic-publication.fish \
            test-foundations.fish test-historical-release-packet.fish test-publication-assets.fish test-release-provenance.fish test-signing-preparer.fish \
