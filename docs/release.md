...
1. the fixed contract-named APK, `Shuba-arm64-v8a.apk`;
...
The APK basename is deliberately independent of the application version so every future publication has a predictable direct-download path while the release tag, Android package metadata, provenance, and checksums retain exact version identity. A public prerelease exposes its APK at `https://github.com/homremwork/shuba/releases/download/v<version>/Shuba-arm64-v8a.apk`. After the same release is promoted, GitHub's native latest-full-release redirect exposes it at `https://github.com/homremwork/shuba/releases/latest/download/Shuba-arm64-v8a.apk`; drafts and prereleases do not change that stable redirect. The historical `v1.0.2` release and its versioned assets remain unchanged, so the stable redirect becomes usable when the next fixed-name candidate is accepted and promoted.
