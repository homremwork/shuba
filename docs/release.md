...
After Android packet and device acceptance, create the unsigned annotated contract tag and build the complete public bundle only through the tracked tooling:

```sh
tools/release/build-exact-source.fish --tag v1.0.2 --output dist/public-source
...
tools/release/verify-publication-bundle.fish --tag v1.0.2 --bundle-dir dist/public-bundle
```

[`build-exact-source.fish`](../tools/release/build-exact-source.fish:1) accepts only the annotated `v<contract-version>` tag and clean initialized recursive checkouts. It exports Git objects at the tag and recorded gitlink commits, rather than copying the worktree. It emits `Shuba-1.0.2-source.tar.zst`, its checksum, a mode/hash inventory, and a recursive commit map. The archive has one `Shuba-1.0.2-source` root, sorted entries, numeric `0:0` ownership, normalized public modes, and the annotated tagger time as its deterministic timestamp. It excludes Git metadata, generated Android/JUCE output, build/release output, unsafe links and paths, local configuration, and signing/private-key material.

[`build-publication-bundle.fish`](../tools/release/build-publication-bundle.fish:1) copies—without rebuilding or changing—the accepted four-file APK packet, exact-source assets, standalone [`LICENSE`](../LICENSE:1) and [`THIRD_PARTY_NOTICES.md`](../THIRD_PARTY_NOTICES.md:1), and reviewed operator-provided release notes. It writes `Shuba-1.0.2-SHA256SUMS`, which hashes every other asset and never itself. Its verifier rejects unexpected inventory, unsafe modes, APK/checksum/provenance/tag mismatches, source-graph drift, private material, and checksum errors. The public GitHub Release must upload exactly the verified bundle bytes.

GitHub’s automatic source archives are not the corresponding-source asset because they omit recursive submodule contents. The current GitHub workflow remains a private rehearsal path, not a publication action; see [`docs/ci.md`](ci.md).
